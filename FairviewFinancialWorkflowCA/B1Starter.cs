...
namespace FairviewFinancialWorkflowCA
{
    public class B1Starter
    {
        ...
        {
            BubbleEvent = true;
            SAPbouiCOM.Form oForm = ProgData.B1Application.Forms.ActiveForm;
            if (ProgData.Forms.ContainsKey(oForm.UniqueID))
            {

                IForm form = ProgData.Forms[oForm.UniqueID];
                if (oForm.TypeEx == "JEF")
                {
                    if ((eventInfo.BeforeAction == true))
                    {
                        ...
                        {
                            SAPbouiCOM.MenuCreationParams oCreationPackage = null;
                            oCreationPackage = ((SAPbouiCOM.MenuCreationParams)(ProgData.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams)));

                            oCreationPackage.Type = SAPbouiCOM.BoMenuType.mt_STRING;
                            oCreationPackage.UniqueID = "linedel";
                            oCreationPackage.String = "Remove Line";
                            oCreationPackage.Enabled = true;
                            oCreationPackage.Position = 0;

                            oMenuItem = ProgData.B1Application.Menus.Item("1280"); // Data'
                            oMenus = oMenuItem.SubMenus;
                    ...
                    else
                    {
                        try
                        {
                            ProgData.B1Application.Menus.RemoveEx("linedel");
                        }
                        catch (Exception ex)
                        {
                            ProgData.B1Application.MessageBox(ex.Message);
                        }
